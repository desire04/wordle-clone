...
class BoardController: NSObject,
  ...
  private func applyNumLettersSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      guard let numLettersSettings = settings[kNumLettersKey] as? Int else {
          assertionFailure("Expecting Int, but got nil")
          return
      }
      self.numItemsPerRow = numLettersSettings
    // END YOUR CODE HERE
  }
  ...
  // Checkpoint: Correctly implementing this should allow you to change the number of rows in the board!
    private func applyNumGuessesSettings(with settings: [String: Any]) {
        // START YOUR CODE HERE
        guard let numGuessesSettings = settings[kNumGuessesKey] as? Int else {
            assertionFailure("Expecting Int, but got nil")
            return
        }
        self.numRows = numGuessesSettings
    // END YOUR CODE HERE
  }
  ...
  private func applyThemeSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      guard let themeSetting = settings[kWordThemeKey] as? String else {
          assertionFailure("Expecting String but was given nil")
          return
      }
      let themeResult = WordTheme(rawValue: themeSetting)
      goalWord = WordGenerator.generateGoalWord(with: themeResult!)
  ...
  private func applyIsAlienWordleSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      if let alienWordleSetting = settings[kIsAlienWordleKey] as? Bool {
          isAlienWordle = alienWordleSetting
      }
    // START YOUR CODE HERE
  }
}
